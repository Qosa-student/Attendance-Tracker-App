...
public class AttendanceActivity extends AppCompatActivity {
    ...
    EditText etSubjectName, etSection, etSchedule, etSearch, etStudentId;
        ...
        ApiService.create().getClasses(userId).enqueue(new Callback<>() {
            ...
            public void onResponse(Call<List<ApiService.ClassModel>> call, Response<List<ApiService.ClassModel>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    android.util.Log.e("AttendanceActivity", "Server error: " + response.code());
                    Toast.makeText(AttendanceActivity.this, "Server error: " + response.code(), Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<ApiService.ClassModel>> call, Throwable t) {
                android.util.Log.e("AttendanceActivity", "Failed to load classes", t);
                Toast.makeText(AttendanceActivity.this, "Error: " + t.getMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    private void updateClassListUI() {
        ...
        for (ApiService.ClassModel cm : classesList) {
            ...
            tvSched.setText("Schedule: " + cm.schedule);
            tvSched.setTextSize(12);
    ...
    private void fetchExistingAttendance() {
        if (selectedClassId == -1) return;
        attendanceMap.clear(); 
        ApiService.create().getTodayAttendance(selectedClassId, today).enqueue(new Callback<List<ApiService.AttendanceRecord>>() {
            @Override
            public void onResponse(Call<List<ApiService.AttendanceRecord>> call, Response<List<ApiService.AttendanceRecord>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    for (ApiService.AttendanceRecord record : response.body()) {
                        attendanceMap.put(record.student_id, record.status);
                    }
                }
                renderStudentList(etSearch.getText().toString().trim());
            }
    ...
    private void searchStudentOnline(String query) {
        ...
        ApiService.create().searchStudents(userId, query).enqueue(new Callback<>() {
            @Override
            public void onResponse(Call<List<ApiService.SearchResultModel>> call, Response<List<ApiService.SearchResultModel>> response) {
                if (currentId != searchRequestId) return;
                if (response.isSuccessful() && response.body() != null) {
                    for (ApiService.SearchResultModel srm : response.body()) {
                        addSearchResult(srm);
                    }
                }
            }
            @Override public void onFailure(Call<List<ApiService.SearchResultModel>> call, Throwable t) {}
        });
    }

    int typedClassId = -1;

    private void addSearchResult(ApiService.SearchResultModel srm) {
        TextView tv = new TextView(this);
        tv.setText(srm.name + " (" + srm.student_number + ") - " + srm.subject_name);
        ...
        tv.setOnClickListener(v -> {
            typedStudent = new ApiService.StudentModel();
            typedStudent.id = srm.id;
            typedStudent.name = srm.name;
            typedStudent.student_number = srm.student_number;
            
            typedClassId = srm.class_id;
            llStudentFound.setVisibility(View.VISIBLE);
            tvFoundName.setText(srm.name);
            tvFoundId.setText(srm.student_number + " | " + srm.subject_name);
            llSearchResults.setVisibility(View.GONE);

            isProgrammaticChange = true;
            etStudentId.setText(srm.name);
    ...
    private void switchTab(int tab) {
        ...
        int activeBg = 0xFFCC0000; 
        int inactiveBg = 0xFFF5F5F5; 
        int activeText = 0xFFFFFFFF; 
        int inactiveText = 0xFFCC0000; 
