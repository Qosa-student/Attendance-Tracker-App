...
public class ManageStudentsActivity extends AppCompatActivity {
    ...
    private void findStudentNameGlobally(String studentNumber) {
        ...
        ApiService.create().searchStudents(userId, studentNumber).enqueue(new Callback<>() {
            @Override
            public void onResponse(Call<List<ApiService.SearchResultModel>> call, Response<List<ApiService.SearchResultModel>> response) {
                if (response.isSuccessful() && response.body() != null && !response.body().isEmpty()) {
                    ApiService.SearchResultModel match = response.body().get(0);
                    if (etStudentName.getText().toString().trim().isEmpty()) {
                        etStudentName.setText(match.name);
                    }
                }
            }
